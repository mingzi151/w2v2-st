...
def count_flops(
    ...
    print("macs: ", all_macs / (repeat * len(sample)))
    print("params: ", params)
...
class SpeechToTextModTaskConfig(SpeechToTextTaskConfig):
    ...
    dropped_decoder_layers: str = field(
        default="-1",
        metadata={"help": "decoder layers to be dropped."}
    )
...
class SpeechToTextModTask(SpeechToTextTask):
    ...
    def __init__(self, cfg, tgt_dict, label_dict):
        ...
        self.dropped_decoder_layers = cfg.dropped_decoder_layers
        self.drop_flag = False
        if self.dropped_decoder_layers != "-1":
            self.dropped_decoder_layers = [int(i) for i in self.dropped_decoder_layers.split("-")]
    ...
    def inference_step(
        ...
        elif self.dropped_decoder_layers != "-1":
            if not self.drop_flag:
                from torch import nn
                decoder_layers = models[0].decoder.layers
                thin_decoder_layers = nn.ModuleList()
                print("decoder layers to be dropped: ", thin_decoder_layers)
                for i, layer in enumerate(decoder_layers):
                    if i not in self.dropped_decoder_layers:
                        thin_decoder_layers.append(layer)
                print("the number of decoder layers at inference: ", len(thin_decoder_layers))
                models[0].decoder.layers = thin_decoder_layers
                self.drop_flag = True
            with torch.no_grad():
