...
class AugmentationNormalizationDataset(BaseWrapperDataset):
    ...
    def __getitem__(self, index: int) -> Tuple[str, torch.tensor, torch.tensor, torch.tensor]:
        ...
        _id, input_tensor, target_tensor, label_tensor, transcript_tensor, mem_index_tensor = self.dataset[index]
        ...
        return (_id, input_tensor, target_tensor, label_tensor, transcript_tensor, mem_index_tensor)

    def _augment(self, input_tensor: torch.tensor) -> torch.tensor:
        # breakpoint()
        # try:
        #     print("!!!!!!!!!!!!!!!!!!!!!!!!!            augmenting dataset: ", [ds.dataset.datasets[0].split for ds in self.dataset.datasets])
        # except Exception:
        #     print("!!!!!!!!!!!!!!!!!!!!!!!!!            augmenting dataset: ", [ds.datasets[0].split for ds in self.dataset.datasets])
    ...
    def _normalize(self, input_tensor: torch.tensor) -> torch.tensor:
        ...
        # print("input_tensor before layer_norm: ", input_tensor)
        # print("input_tensor.var before layer_norm: ", input_tensor.var())
        with torch.no_grad():
            input_tensor = F.layer_norm(input_tensor, input_tensor.shape)
        # print("input_tensor.var after layer_norm: ", input_tensor.var())
        # print("=" * 100)
        return input_tensor
